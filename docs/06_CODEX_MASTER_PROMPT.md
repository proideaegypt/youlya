...
## VIBE CODE WORKFLOW — MANDATORY LOOP

After completing EVERY task, execute this loop automatically without being asked:

```
PROMPT → WORK → TESTS → PUSH → VERSION → README → (REPULL) → DONE YA BOSS ✅
```
...
```
If `.env`, `.env.local`, `.env.production`, `*.key`, `*.pem` appear → STOP. Remove from staging. Never commit secrets.

### Version & Tag
```bash
git add .
git commit -m "[<phase>] <task-slug> — <summary>"
git tag -a vX.Y.Z-<version-name> -m "<summary>"
git push origin main --tags
```
Version name = human-readable task slug. Example: `v0.2.0-order-confirmation-idempotency`

### Release File
Create `RELEASES/<version-name>.md` with: task prompt, what was done, files changed, test results, phase progress, next step, status ✅ DONE YA BOSS.
Append one line to root `README.md` changelog.

### Portainer Repull (Phase 2+ Dashboard only)
```bash
...
### Final Output (every task, always)
```
✅ DONE YA BOSS
