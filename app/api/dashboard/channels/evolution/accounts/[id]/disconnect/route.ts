...
import { logoutEvolutionInstance } from "@/lib/adapters/evolution/evolution-instance-client";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (account.evolution_instance) {
      await logoutEvolutionInstance(account.evolution_instance);
    }
    await updateChannelAccount(id, { status: "disconnected", qr_status: "unknown" });
