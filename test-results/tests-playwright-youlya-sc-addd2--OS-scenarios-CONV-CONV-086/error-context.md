...
- Name: tests/playwright-youlya-scenarios.spec.ts >> Youlya AI Commerce OS scenarios (CONV) >> CONV-086
- Location: tests/playwright-youlya-scenarios.spec.ts:77:9

# Error details

```
Error: CONV-086 action

expect(received).toBe(expected) // Object.is equality

Expected: "duplicate_ignored"
Received: "product_results"
```

# Test source

```ts
  73  |     expect(INTERNAL_API_SECRET, 'INTERNAL_API_SECRET is required for internal contract tests').toBeTruthy();
  74  |   });
  75  | 
  76  |   for (const scenario of scenarios) {
  77  |     test(`${scenario.id}`, async ({ request }) => {
  78  |       test.skip(
  79  |         scenario.id.startsWith('DASH-'),
  80  |         'Dashboard scenarios are Phase 2 and must use dashboard routes, not /api/internal/messages/turn.',
  81  |       );
  82  | 
