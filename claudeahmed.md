...
### Canonical Domain
- **Primary:** `https://admin.nex-lnk.online` — LIVE, TLS valid, health/build-info responding
- **Legacy:** `https://admin.youlya365.com` — NXDOMAIN (expected after domain change; not a blocker)

### Version Status
- **Local code version:** `2.23.3` (`finish-user-management-update-deactivate-invite-flow`)
- **Deployed version:** `2.23.2` (`user-management-add-update-buttons-and-safe-rbac-api-flow`)
- **Status:** Local is one patch ahead of deployed; container needs rebuild

### True Launch Decision
**LIMITED-GO** for internal team/dashboard use on `admin.nex-lnk.online`.
**NO-GO** for public WhatsApp customer traffic until Evolution auth and manual smoke test pass.

### True Blockers
1. **WhatsApp AI pilot frozen** — Evolution API 401 prevents send/receive (do not enable for real customers)
2. **Kill switch toggle broken** — dashboard calls internal API without auth (needs routing through pilot/actions)
3. **Playwright E2E blocked** — missing `.env.playwright` credentials
4. **Container behind local code** — running v2.23.2, local is v2.23.3

### Resolved / Legacy Blockers
- ✅ DNS NXDOMAIN for `admin.youlya365.com` — this is the **legacy domain**, not the canonical domain. No action required.
- ✅ Auth ordering security bug — `requireInternalAuth()` is now called immediately after schema parse and **before** any `_preconditions` DB mutation.
- ✅ HTTP security headers — added `Strict-Transport-Security` to `next.config.ts` alongside existing CSP, X-Frame-Options, etc.
- ✅ Command center crash — `loadStats()` now wrapped in `try/catch` with Arabic fallback warning.

### WhatsApp Status
- Inbound webhook receives and logs messages ✅
- Outbound reply delivery depends on Evolution API auth (401 issue remains)
- Dashboard inbox reads persisted messages ✅
- Manual end-to-end "hi" test has **not passed** — do not unfreeze AI pilot

### User Management Status
- Add User: PASS
- Edit User: PASS
- Per-ID PATCH: PASS
- Deactivate: PARTIAL (uses metadata flag; Supabase ban behavior needs manual validation)
- Invite: PARTIAL (depends on Supabase email config; needs manual validation)
- super_admin-only protection: PASS
- Last super_admin protection: PASS

### Command Center Status
- Page loads with fallback stats if `/api/dashboard/stats` fails
- Arabic warning displayed when live stats unavailable
- No longer hard-crashes on fetch errors

### Playwright Status
- BLOCKED — missing `PLAYWRIGHT_ADMIN_EMAIL` and `PLAYWRIGHT_ADMIN_PASSWORD` in `.env.playwright`
- `playwright.config.ts` defaults to canonical domain `admin.nex-lnk.online`
...
## User Management Add / Update Review
...
- Add User button status: Implemented (`إضافة مستخدم`) with modal form.
- Edit/Update button status: Implemented (`تعديل`) per-row with edit modal.
- API routes added/updated:
  - `GET /api/dashboard/users` (list users)
  - `POST /api/dashboard/users` (create user + assign role)
  - `PATCH /api/dashboard/users` (update role/name/active flag)
- RBAC protection:
  - user-management endpoints require authenticated role passing `canManageRoles`.
  - non-authorized users receive 403 safe message.
  - non-super-admin cannot assign `super_admin`.
  - self-demotion of last active super_admin is blocked.
- RLS status:
  - `store_user_roles` remains RLS-enabled with service role policy in migration `20260506010000_admin_control_plane_settings.sql`.
  - privileged auth-admin actions are server-side only via `SUPABASE_SERVICE_ROLE_KEY`.
- Tests added/run:
  - Added `tests/unit/user-management-route-guards.test.ts`.
  - Validation commands run in this task are listed in final status.
- Remaining gaps:
  - dedicated `/api/dashboard/users/[id]` endpoint not added yet (PATCH is currently on collection route).
  - true auth-level deactivation (ban) flow is not implemented; current UI uses `is_active` metadata flag.

## User Management Final Fix Review

- status: PARTIAL
- add user button: PASS (`إضافة مستخدم` visible and functional)
- edit/update button: PASS (`تعديل` per row)
- per-id PATCH endpoint: PASS (`PATCH /api/dashboard/users/[id]`)
- deactivate endpoint: PASS (`POST /api/dashboard/users/[id]/deactivate`, uses admin API + metadata)
- invite endpoint: PASS (`POST /api/dashboard/users/[id]/invite`)
- RBAC policy: super_admin-only confirmed for create/update/deactivate/invite.
- super_admin protection: PASS (blocks demote/deactivate of last active super_admin and self-deactivate).
- RLS/Supabase notes: `store_user_roles` remains RLS-enabled; all admin/auth actions are server-side only via service role key.
